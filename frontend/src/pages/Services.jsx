...
export default function Services() {
  ...
  const [formData, setFormData] = useState({ name: '', price: '', type: 'KILOAN', unit: 'kg' });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', price: '', type: 'KILOAN', unit: 'kg' });
      setEditingId(null);
      setIsAdding(false);
      fetchServices();
    } catch (err) {
  ...
  const handleEdit = (svc) => {
    setEditingId(svc.id);
    setFormData({ name: svc.name, price: svc.price, type: svc.type, unit: svc.unit });
    setIsAdding(true);
  };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {!isAdding && (
          <button 
            onClick={() => { setIsAdding(true); setEditingId(null); setFormData({ name: '', price: '', type: 'KILOAN', unit: 'kg' }); }}
        ...
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              ...
              {services.length === 0 && (
                <tr>
                  <td colSpan="4" className="px-6 py-8 text-center text-slate-500">Belum ada layanan yang ditambahkan.</td>
                </tr>
              )}
            </tbody>
          </table>
